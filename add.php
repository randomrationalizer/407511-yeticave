...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!isset($_SESSION["user"])) {
        ...
        $page_content = include_template("error.php", ["error_header" => "403 Доступ запрещен", "error_text" => "Доступ к данной странице разрешен только зарегистрированным пользователям."]);
        $layout_content = include_template("layout.php", ["page_content" => $page_content, "page_title" => "403 Доступ запрещен", "categories" => $categories]);
        print($layout_content);
    ...
    $layout_content = include_template("layout.php", ["page_content" => $page_content, "page_title" => "Добавить лот", "categories" => $categories]);
    print($layout_content);
}
