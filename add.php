...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    foreach ($requered_fields as $field) {
        if (!isset($new_lot[$field]) || empty(trim($new_lot[$field]))) {
    ...
    foreach ($integer_fields as $field) {
        if (!empty($new_lot[$field]) && (!filter_var($new_lot[$field], FILTER_VALIDATE_INT) || intval($new_lot[$field]) < 0)) {
    ...
    if (isset($_FILES["lot-photo"]["name"]) && !empty($_FILES["lot-photo"]["name"])) {
        ...
        $file_type = finfo_file($finfo, $_FILES["lot-photo"]["tmp_name"]);
    ...
    if (empty($errors)) {
        ...
        $stmt = db_get_prepare_stmt($link, $sql_add_lot, [
            $new_lot['name'],
            $new_lot['description'],
            $new_lot["img_path"],
            $new_lot["start_price"],
